...
module.exports = function (moduleOptions) {
  ...
  function getImageUrls (string) {
    ...
    const test = new RegExp(
      '(?:http(s?):)(//[^"\']*?.(?:' + options.extensions.map(ext => `\\.${ext}`).join('|') + '))',
      'gi'
    )
    const matches = string.matchAll(test)
    let baseUrl
    if (options.baseUrl) {
      // ensure baseUrl has protocol to avoid invalid URL errors
      // we're only using URL to compare hostnames so the protocol is irrelevant
      baseUrl = new URL(ensureURLHasProtocol(options.baseUrl))
    }
    for (const match of matches) {
      ...
      try {
        ...
        if (options.verbose)
          consola.warn(`${consoleTitle} invalid url: ${match[0]}`)
        continue
      }
      if (baseUrl) {
  ...
  async function saveRemoteImages () {
    ...
    if (warnings.length) {
      ...
      warnings.forEach((w) => {
        consola.log(`${w.href} [${chalk.bold(w.code)}]`)
      })
    }
