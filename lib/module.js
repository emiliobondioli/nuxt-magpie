...
module.exports = function (moduleOptions) {
  ...
  async function saveRemoteImages () {
    ...
    if(warnings.length) {
      consola.warn(`${consoleTitle} ${chalk.bold(warnings.length)} errors downloading images:`)
      warnings.forEach(w => {
    ...
    return new Promise((resolve, reject) => {
      const req = agent
        .request(href, (response) => {
          ...
          if(response.statusCode !== 200) warnings.push({href, code: response.statusCode, message: response.statusMessage})
